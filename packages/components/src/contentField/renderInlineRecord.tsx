...
export const renderInlineRecord = ({
  record,
}: RenderInlineRecordContext<CdaStructuredTextRecord | PageLinkFragment | ConcertLinkFragment>) => {
  if (isOfType<PageLinkFragment>(record, "PageRecord")) {
    if (!record.slug) return null;
    return <Link href={slugFormatter({ slug: record.slug })}>{record.title}</Link>;
  }
