...
export const getConcertPageSeo = async ({ slug }: GetConcertSeoQueryVariables) => {
  const { metadata } = await getSiteMetadata();

  try {
    const { data, error } = await client.query<GetConcertSeoQuery, GetConcertSeoQueryVariables>(
      GetConcertSeoDocument,
      { slug },
    );
